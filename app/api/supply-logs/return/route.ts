// app/api/supply-logs/return/route.ts - Return supply from employee
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: ReturnSupplyRequest = await request.json();
    const { logId, status, notes } = body;

    if (!logId || !['returned', 'damaged', 'lost'].includes(status)) {
    ...
    const updateLogStmt = db.prepare(`
      UPDATE supply_logs
      SET status = ?, returned_date = ?, notes = ?, received_by = ?, updated_at = ?
      WHERE id = ?
    `);

    updateLogStmt.run(status, now, notes || null, user.id, now, logId);

    // If status is 'returned', restore supply quantity
    if (status === 'returned') {
    ...
    console.error('Return supply error:', error);
    return NextResponse.json(
